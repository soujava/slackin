...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuários online neste momento de ',
          dom('b.total', total), ' usuários registrados.'
        ]
        : [dom('b.total', total), ' usuários estão registrados neste slack até o momento.']
    ),
    dom('form id=invite',
      channels && (
        channels.length > 1
      ...
      dom('input.form-item type=email name=email placeholder=voce@seudominio.com'
        + (!iframe ? '' : '')),
      coc && dom('.coc',
        dom('label',
      ...
      dom('button.loading', 'Pedir convite para entrar')
    ),
    dom('p.signin',
      'ou ',
      dom(`a href=https://${org}.slack.com target=_top`, 'fazer login com conta existente no Slack'),
      '.'
    ),
    !iframe && dom('footer',
...
function style ({ logo, active, large, iframe } = {}){
  var css = dom.style()
  css.add('html', { 'font-size': large ? '16px' : '12px', 
                    'background-image': 'url(assets/javadevbr-background.jpg)',
  ...
  css.add('p', {
    'font-size': iframe ? '1.5rem' : '2.0rem',
    'margin': iframe ? '0 0 .8rem' : '.8rem 0',
    'color': 'white'
  })
